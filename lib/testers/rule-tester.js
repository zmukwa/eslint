...
RuleTester.prototype = {
    ...
    run(ruleName, rule, test) {
        ...
        function testValidTemplate(ruleName, item) {
            ...
            assert.equal(messages.length, 0, util.format(
        ...
        function testInvalidTemplate(ruleName, item) {
            ...
            if (typeof item.errors === "number") {
                assert.equal(messages.length, item.errors, util.format(
                ...
                assert.equal(messages.length, item.errors.length, util.format(
                    "Should have %d error%s but had %d: %s",
                    item.errors.length,
