...
function mergeFixes(fixes, sourceCode) {
    ...
    for (const fix of fixes) {
        ...
        // A fix to lock range is `text: null`.
        // In this case, it updates the end location of the merged range,
        // does not check overlapping in the fixes.
        if (fix.text === null) {
            end = Math.max(end, fix.range[1]);
            continue;
        }
